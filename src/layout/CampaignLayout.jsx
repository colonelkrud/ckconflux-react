...
export default function CampaignLayout({ children, label, footerNavLabel = 'Campaign page links' }) {
  return <div className="min-h-screen overflow-x-hidden bg-slate-950 text-slate-100">
    ...
    <footer className="border-t border-white/10"><nav aria-label={footerNavLabel} className="mx-auto flex max-w-5xl flex-wrap justify-center gap-x-5 gap-y-2 px-4 py-7 text-sm text-slate-400">{links.map(([linkLabel, to]) => <Link key={to} to={to} className="rounded hover:text-white">{linkLabel}</Link>)}</nav></footer>
  </div>;
}
