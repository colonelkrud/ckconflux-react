  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto grid w-full max-w-6xl gap-8 px-4 py-14 sm:px-6 md:py-20 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-24">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 max-w-xl text-base leading-7 text-slate-300 sm:text-lg">Element on Matrix is the primary CK Conflux experience. Use Element Call for integrated voice and video; optional services stay secondary.</p>
      ...
      <section id="tools" className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-200 sm:text-sm">Primary calling experience</p>
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Element Call, built into Matrix rooms</h2>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <article className="rounded-2xl border border-white/10 bg-white/5 p-5">
            <h3 className="text-lg font-semibold text-white">Voice, video, and screen sharing</h3>
            <p className="mt-2 text-sm leading-6 text-slate-300">Move from a Matrix room into an Element Call with the identity you already use.</p>
            <Link to="/calls" className="mt-4 inline-flex rounded-xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950">Explore Element Call</Link>
          </article>
          <article className="rounded-2xl border border-white/10 bg-white/5 p-5">
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Optional secondary service</p>
            <h3 className="mt-2 text-lg font-semibold text-white">TeamSpeak 6 Beta</h3>
            <p className="mt-2 text-sm leading-6 text-slate-300">A separate beta voice workflow for members who specifically need it.</p>
            <Link to="/teamspeak" className="mt-4 inline-flex rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white">TeamSpeak details</Link>
          </article>
        </div>
      </section>
