...
describe('CK Conflux application architecture', () => {
  ...
  it('documents the first-five-minutes production room and notice contracts', () => {
    renderPath('/help');
    const quickStart = screen.getByRole('heading', { name: 'Your first five minutes in CK Conflux' }).closest('section');
    expect(quickStart).toHaveTextContent('#welcome:ckconflux.com');
    expect(quickStart).toHaveTextContent('#general:ckconflux.com');
    expect(quickStart).toHaveTextContent('#help-and-support:ckconflux.com');
    expect(quickStart).toHaveTextContent(/CK Conflux-specific community room for service and help questions/i);
    expect(quickStart).toHaveTextContent(/Server Notices are generated by the CK Conflux homeserver/i);
    expect(screen.getByRole('link', { name: 'independent status page' })).toHaveAttribute('href', 'https://status.ckconflux.com');
    expect(screen.getByRole('link', { name: 'Element Call guide' })).toHaveAttribute('href', '/calls');
  });

  it('recommends Element X and delegates generic client skills to first-party guides', () => {
    renderPath('/help');
    expect(screen.getByText(/Element X is CK Conflux’s recommended current mobile client for iOS and Android/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'official Element download page' })).toHaveAttribute('href', 'https://element.io/download');
    expect(screen.getByRole('link', { name: 'Element onboarding guide' })).toHaveAttribute('href', expect.stringMatching(/^https:\/\/docs\.element\.io\//));
    expect(screen.getByRole('link', { name: 'Matrix chat basics' })).toHaveAttribute('href', expect.stringMatching(/^https:\/\/matrix\.org\//));
  ...
  it('distinguishes recovery credentials and the two verification concepts', () => {
    ...
    expect(screen.getByText(/Password recovery does not recreate encryption keys/i)).toBeInTheDocument();
    expect(screen.getByText(/CK Conflux cannot recreate a lost cryptographic recovery key/i)).toBeInTheDocument();
  ...
  it('hands successful registration to the anchored quick start', () => {
    renderPath('/join');
    expect(screen.getByRole('link', { name: 'Continue with your first five minutes' })).toHaveAttribute('href', '/help');
  });
